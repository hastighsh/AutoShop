 ...
 * does NOT have two vehicles that are equal to each other.
 * </p>
 */
public class AutoShop {
     ...
     *
     * <p>
     * Returning the owner of this auto shop allows any user to sell vehicles from the auto shop (because any
     ...
     *
     * <p>
     * Assume that the input list does NOT have any duplicate.
     * </p>
     *
    ...
    public void add(List<Vehicle> vehicles) {
        ...
        for (int i = 0; i < vehicles.size(); i++) {
            if (this.vehicles.contains(vehicles.get(i))) {
                throw new IllegalArgumentException("do NOT add any vehicle to the shop");
            } else {
                temp.add(vehicles.get(i));
            }
        }
        this.vehicles.addAll(temp);
    ...
    public Vehicle sellingSingleVehicle(ShopOwner user, Vehicle vehicle) {
        if (!vehicles.contains(vehicle) || owner != user) {
            return null;
        }
        return vehicle;
    }
    ...
    public List<Vehicle> deepCopy() {
        ...
        for (Vehicle obj:vehicles) {
            temp = new Vehicle(obj.getMake(), obj.getYearMake(), obj.getNumOfDoors(), obj.getPrice());
            newList.add(temp);
        }
    ...
    public List<Vehicle> shallowCopy() {
        List<Vehicle> newList = new ArrayList<>(vehicles);
        Collections.sort(newList);
        return newList;
    }
